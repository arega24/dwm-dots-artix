...
usbdrives="$(lsblk -rnpo name,size,mountpoint,fstype | grep 'sda' | awk 'NF==3 {print $1, $2, $3}')"
mounteddrives="$(lsblk -rnpo name,size,mountpoint,fstype | grep 'sda' | awk 'NF==4 {print $1, $2, $3, $4}')"

menu(){ \
	epilogimenu=$(echo -e "Mount\nUmount\nUmountAll" | rofi -dmenu -show run -lines 3 -opacity "85" -bw 0 -width 30 -padding 20 -i -p "USB Name " | awk '{print $1}')
}
